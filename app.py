...
def get_accommodation_options(property_obj):
    """Return logical accommodation inventory configured for the property."""
    ...
    if ptype == "resort":
        ...
        for key, count in details.get("room_types", {}).items():
            ...
            if not price:
                continue
            options.append({"key": str(key), "label": labels.get(key, key.replace("_", " ").title()),
        ...
        for key, count in details.get("flat_types", {}).items():
            ...
            if not price:
                continue
            options.append({"key": str(key), "label": labels.get(key, key.upper()),
...
def get_inventory_blocked_counts(property_obj, check_in, check_out):
    ...
    counts = {}
    for row in rows:
        base = row.inventory_key.rsplit(":", 1)[0] if ":" in row.inventory_key else row.inventory_key
...
def approve_booking(booking_id):
    ...
    if existing_approved_booking and booking.property.property_type == "guesthouse":
        flash(
...
def type3_available(booking_id):
    ...
    if property_obj.property_type == "guesthouse":
        if not is_property_available(property_obj.id, booking.check_in, booking.check_out, exclude_booking_id=booking.id):
            flash("The requested dates are no longer available.", "danger")
            return redirect(url_for("admin_dashboard"))
    else:
        ok, message, normalized, _ = validate_accommodation_selection(property_obj, get_booking_accommodations(booking), booking.guests, booking.check_in, booking.check_out, exclude_booking_id=booking.id)
        if not ok:
            flash(message or "The selected accommodation is no longer available.", "danger")
            return redirect(url_for("admin_dashboard"))
