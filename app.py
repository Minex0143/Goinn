...
def is_property_available(
    ...
    # A pending, availability-requested or approved booking
    # blocks the property for overlapping dates. Rejected
    # bookings do not block the dates.
    ...
    # When Admin is approving a Type 3 request, the booking
    # itself must be excluded from the overlap check.
...
def build_booking_whatsapp_url(booking):
    """Build the same WhatsApp message used when a booking is submitted."""
...
"""
        ...
        whatsapp_contact = owner.contact if owner else ""
...
def property_details(property_id):
    ...
    # --------------------------------------------------
    # CHECK FOR AN EXISTING BOOKING REQUEST
    # --------------------------------------------------
    # Once the user has submitted the booking and been sent
    # to WhatsApp, the booking record itself keeps the dates
    # reserved. If the user returns to the browser without
    # sending the WhatsApp message, do not show the
    # confirmation popup again. Show the existing request
    # and provide the same WhatsApp message again instead.
    booking_request = None
    booking_request_sent = False
    booking_whatsapp_url = None
    ...
    if (
        ...
        try:
            request_check_in = datetime.strptime(
                selected_check_in,
                "%Y-%m-%d"
            ).date()
            request_check_out = datetime.strptime(
                selected_check_out,
                "%Y-%m-%d"
            ).date()
            ...
            booking_request = Booking.query.filter(
                ...
                Booking.check_in == request_check_in,
                Booking.check_out == request_check_out
            ).order_by(
...
def book_property(property_id):
    ...
    # ==================================================
    # EXISTING REQUEST FROM THIS USER
    # ==================================================
    # Prevent duplicate booking records if the user returns
    # to the booking flow or submits the confirmation twice.
    existing_user_request = Booking.query.filter(
        Booking.user_id == current_user.id,
        Booking.property_id == property_obj.id,
        Booking.status.in_([
            "pending",
    ...
    # Every submitted booking request reserves the dates.
    # This also applies to Type 3 availability requests.
    ...
    if not is_property_available(
        ...
        return render_template(
            ...
            tomorrow=tomorrow_string,
            error=(
                "Sorry, this property already has a booking request "
                "for the selected dates. Please choose different dates."
            )
        )
    ...
    # The same URL is also generated later from the saved
    # booking request, so the user can return and click
    # WhatsApp again without creating another booking.
