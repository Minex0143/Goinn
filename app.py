...
class Property(db.Model):
    ...
    # ------------------------------------------------
    # ADMIN PROPERTY TYPE / CHECK-IN CHECK-OUT DETAILS
    # ------------------------------------------------
...
# ==================================================
# PROPERTY FEATURE IMAGE MODEL
# ==================================================

class PropertyFeatureImage(db.Model):

    __tablename__ = "property_feature_images"
    ...
    unit_index = db.Column(db.Integer, nullable=False, default=1)
    filename = db.Column(db.String(255), nullable=False)
    mime_type = db.Column(db.String(100), nullable=False)
    image_data = db.Column(db.LargeBinary, nullable=False)
...
def ensure_database_columns():
    ...
    try:
        ...
        # ------------------------------------------
        # PROPERTY TYPE / DETAILS / CHECK-IN-OUT
        # ------------------------------------------

        for column_name, sql_type in [
            ("property_type", "VARCHAR(40)"),
            ("property_type_data", "TEXT"),
            ...
            if column_name not in columns:
                with db.engine.begin() as connection:
                    connection.exec_driver_sql(
                        f"ALTER TABLE property ADD COLUMN {column_name} {sql_type}"
                    )
...
def get_property_booking_state(
    ...
    """Return the public booking state for selected dates.
...
def property_availability_check(property_id):
    ...
    if not is_property_available(property_obj.id, check_in, check_out):
...
def property_details(property_id):
        ...
        try:
            ...
            if requested_out_date <= requested_in_date:
                ...
                if requested_state != "available":
                    ...
                    and int(requested_guests) > property_obj.max_guests
                ):
                    edit_error = (
    ...
    property_type_data = {}
    try:
        property_type_data = json.loads(property_obj.property_type_data or "{}")
        if not isinstance(property_type_data, dict):
            property_type_data = {}
    except Exception:
        property_type_data = {}
...
def delete_owner_profile():
    ...
    try:
        ...
        for property_obj in owner_properties:
            ...
            PropertyFeatureImage.query.filter_by(
                property_id=property_obj.id
            ).delete(synchronize_session=False)
...
def admin_type2_block_date(property_id):
    ...
    booking = Booking.query.filter(
        Booking.property_id == property_id,
        Booking.status.in_(["pending", "approved", "availability_requested"]),
...
# ==================================================
# ADMIN PROPERTY TYPE / DETAILS
# ==================================================

PROPERTY_TYPES = {
    "guesthouse_farmhouse": "Guesthouse / Farmhouse",
    "resort": "Resort",
    "hotel": "Hotel",
...
HOTEL_ROOM_TYPES = [
    "king_bed", "queen_bed", "twin_bed", "suite", "superior", "deluxe",
    "family", "executive", "studio", "triple", "accessible"
]

SERVICE_APARTMENT_TYPES = ["1bhk", "2bhk", "3bhk", "4bhk", "5bhk", "6bhk"]


def _positive_int(value, label, maximum=500):
    try:
        number = int(value)
    except (TypeError, ValueError):
...
@app.route("/admin/property/<int:property_id>/property-type", methods=["POST"])
@login_required
def save_property_type(property_id):
    ...
    try:
        ...
        if property_type == "guesthouse_farmhouse":
            data["bedrooms"] = _positive_int(request.form.get("bedrooms"), "Total bedrooms")
            data["washrooms"] = _positive_int(request.form.get("washrooms"), "Total washrooms")
            data["kitchen_access"] = request.form.get("kitchen_access", "no")
            data["swimming_pool_access"] = request.form.get("swimming_pool_access", "no")
            data["extra_beds"] = request.form.get("extra_beds", "no")

        elif property_type == "resort":
            total = _positive_int(request.form.get("total_properties"), "Total properties", 200)
            if total < 1:
                raise ValueError("A resort must have at least 1 property/unit.")
            units = []
            for i in range(1, total + 1):
                units.append({
                    "bedrooms": _positive_int(request.form.get(f"unit_{i}_bedrooms"), f"Unit {i} bedrooms"),
                    "washrooms": _positive_int(request.form.get(f"unit_{i}_washrooms"), f"Unit {i} washrooms"),
                    "kitchen_access": request.form.get(f"unit_{i}_kitchen_access", "no"),
                    "swimming_pool_access": request.form.get(f"unit_{i}_swimming_pool_access", "no"),
                    "extra_beds": request.form.get(f"unit_{i}_extra_beds", "no"),
                })
            data["total_properties"] = total
            data["units"] = units

        elif property_type == "hotel":
            rooms = {}
            for room_type in HOTEL_ROOM_TYPES:
                rooms[room_type] = _positive_int(
                    request.form.get(f"room_{room_type}"),
                    room_type.replace("_", " ").title()
                )
            data["rooms"] = rooms
            data["total_rooms"] = sum(rooms.values())
            if data["total_rooms"] < 1:
                raise ValueError("A hotel must have at least 1 room.")

        elif property_type == "service_apartment":
            flats = {}
            for flat_type in SERVICE_APARTMENT_TYPES:
                flats[flat_type] = _positive_int(
                    request.form.get(f"flat_{flat_type}"),
                    flat_type.upper() + " flats"
                )
            data["flats"] = flats
            data["total_flats"] = sum(flats.values())
            if data["total_flats"] < 1:
                raise ValueError("A service apartment must have at least 1 flat.")

        property_obj.property_type = property_type
        property_obj.property_type_data = json.dumps(data, ensure_ascii=False)
        db.session.commit()
        flash("Property type and accommodation details saved. Continue to check-in/check-out timings.", "success")
    except ValueError as error:
        db.session.rollback()
        flash(str(error), "danger")
    except Exception as error:
        db.session.rollback()
        print("PROPERTY TYPE ERROR:", error)
        flash("Unable to save property type details.", "danger")

    return redirect(url_for("admin_dashboard", tab="pending-properties"))


@app.route("/admin/property/<int:property_id>/timings", methods=["POST"])
@login_required
def save_property_timings(property_id):
    if not current_user.is_admin:
        return "Unauthorized", 403
    ...
    check_in = request.form.get("check_in_time", "").strip()
    check_out = request.form.get("check_out_time", "").strip()
    import re
    time_pattern = re.compile(r"^(?:[01]\d|2[0-3]):[0-5]\d$")
    if not time_pattern.match(check_in) or not time_pattern.match(check_out):
        flash("Please select valid check-in and check-out times.", "danger")
        return redirect(url_for("admin_dashboard", tab="pending-properties"))

    property_obj.check_in_time = check_in
    property_obj.check_out_time = check_out
    db.session.commit()
    flash("Check-in and check-out timings saved. Continue to pricing.", "success")
    return redirect(url_for("admin_dashboard", tab="pending-properties"))


@app.route("/admin/property/<int:property_id>/feature-image/<category>/<int:unit_index>/upload", methods=["POST"])
@login_required
def upload_property_feature_image(property_id, category, unit_index):
    if not current_user.is_admin:
        return "Unauthorized", 403
    property_obj = db.session.get(Property, property_id)
    if not property_obj:
    ...
    image = PropertyFeatureImage.query.filter_by(
        property_id=property_id, category=category, unit_index=unit_index
    ).first()
    if not image:
        image = PropertyFeatureImage(
            property_id=property_id, category=category, unit_index=unit_index
        )
        db.session.add(image)
    image.filename = file.filename
    ...
    image.image_data = data
    db.session.commit()
    flash("Feature image uploaded successfully.", "success")
    return redirect(url_for("admin_dashboard", tab="pending-properties"))


@app.route("/admin/property-feature-image/<int:image_id>/view")
@login_required
def admin_view_property_feature_image(image_id):
    if not current_user.is_admin:
        return "Unauthorized", 403
    image = db.session.get(PropertyFeatureImage, image_id)
    if not image:
        return "Image not found", 404
    return app.response_class(image.image_data, mimetype=image.mime_type,
                              headers={"Content-Disposition": "inline"})


@app.route("/property-feature-image/<int:image_id>")
def property_feature_image(image_id):
    image = db.session.get(PropertyFeatureImage, image_id)
    if not image:
        return "Image not found", 404
    property_obj = db.session.get(Property, image.property_id)
...
def set_property_price(property_id):
    ...
    if not property_obj.property_type or not property_obj.property_type_data:
        return "Property type and accommodation details must be configured before approval.", 400

    if not property_obj.check_in_time or not property_obj.check_out_time:
        return "Check-in and check-out timings must be configured before approval.", 400
