...
def get_accommodation_options(property_obj):
    """Return the configured selectable accommodation inventory."""
    ...
    if ptype == "resort":
        for unit in details.get("units", []):
            ...
            if not price:
                continue
            options.append({"key": f"unit:{idx}", "label": f"Unit {idx}", "group": "Resort Unit",
                            "count": 1, "max_guests": int(price.get("max_guests", 0)),
                            "price": float(price.get("price", 0))})
    elif ptype == "hotel":
        labels = dict(HOTEL_ROOM_TYPES)
        for key, count in details.get("room_types", {}).items():
            if int(count or 0) <= 0:
                continue
            price = pricing.get(key, {}) if isinstance(pricing, dict) else {}
            if not price:
                continue
            options.append({"key": key, "label": labels.get(key, key.replace("_", " ").title()),
                            "group": "Hotel Room", "count": int(count),
                            "max_guests": int(price.get("max_guests", 0)),
                            "price": float(price.get("price", 0))})
    elif ptype == "service_apartment":
        labels = dict(FLAT_TYPES)
        for key, count in details.get("flat_types", {}).items():
            if int(count or 0) <= 0:
                continue
            price = pricing.get(key, {}) if isinstance(pricing, dict) else {}
            if not price:
                continue
            options.append({"key": key, "label": labels.get(key, key.upper()),
                            "group": "Service Apartment", "count": int(count),
                            "max_guests": int(price.get("max_guests", 0)),
                            "price": float(price.get("price", 0))})
    return options
...
def get_property_total_capacity(property_obj):
    ...
    if property_obj.property_type in (None, "", "guesthouse"):
        return int(property_obj.max_guests or 0)
    return sum(max(0, int(o.get("count", 0))) * max(0, int(o.get("max_guests", 0)))
                 for o in get_accommodation_options(property_obj))


def get_booking_accommodations(booking):
    try:
        data = json.loads(booking.selected_accommodations or "[]")
        return data if isinstance(data, list) else []
    except Exception:
        return []


def get_reserved_accommodation_counts(property_obj, check_in, check_out):
    """Count selected physical inventory for overlapping active bookings."""
    result = {}
...
def validate_accommodation_selection(property_obj, selected, guests, check_in=None, check_out=None):
    """Validate the cart and return (ok, error, normalized, price_per_day)."""
    ...
    reserved = get_reserved_accommodation_counts(property_obj, check_in, check_out) if check_in and check_out else {}
    normalized = []
    capacity = 0
    total_price = 0.0
    for item in selected:
        ...
        if qty < 1:
            continue
        opt = options[key]
        available = int(opt["count"]) - int(reserved.get(key, 0))
        if qty > available:
            return False, f"Only {available} {opt['label']} available for the selected dates.", [], None
...
def get_property_booking_state(
    ...
    approved_booking = approved_query.first()
    ...
    pending_booking = pending_query.first()
...
def property_accommodation_availability(property_id):
    ...
    expire_booking_holds()
    reserved = get_reserved_accommodation_counts(property_obj, check_in, check_out)
    options = []
    for opt in get_accommodation_options(property_obj):
        options.append({**opt, "available": max(0, int(opt["count"]) - int(reserved.get(opt["key"], 0)))})
    return jsonify({"property_type": property_obj.property_type, "options": options})
...
def admin_type2_calendar(property_id):
    ...
    # Availability management is supported for all approved property types.
...
def type3_available(booking_id):
    ...
    # Release any stale temporary holds before checking the requested dates.
    # The current Type 3 request is then excluded from its own availability check.
    expire_booking_holds()

    # The availability request itself temporarily freezes these dates.
    # When the admin approves that same request, exclude it from the
    # availability check; otherwise the request would incorrectly
    # make itself unavailable. Other approved/pending requests and
    # owner-blocked dates are still respected.
...
def set_property_price(property_id):
    ...
    time_pattern = re.compile(r"^([01]?[0-9]|2[0-3]):[0-5][0-9]$")
    if not time_pattern.match(check_in_time) or not time_pattern.match(check_out_time):
        return "Please select valid check-in and check-out timings.", 400
