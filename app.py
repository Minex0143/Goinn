...
# Booking email is disabled by default so the application works on
# Render plans where outbound SMTP is unavailable. Set
# ENABLE_BOOKING_EMAIL=true later when email is ready to be enabled.
app.config["ENABLE_BOOKING_EMAIL"] = (
    os.environ.get("ENABLE_BOOKING_EMAIL", "false").strip().lower()
    in ("1", "true", "yes", "on")
)
...
# ==================================================
# LOST ACCOUNT RECOVERY REQUEST
# ==================================================

class LostAccountRequest(db.Model):
    ...
    previous_email = db.Column(
        db.String(160),
        nullable=False
    )

    previous_contact = db.Column(
        db.String(30),
        nullable=False
    )
...
# ==================================================
# LOST ACCOUNT HELPERS
# ==================================================

def normalize_recovery_contact(value):
...
def valid_recovery_contact(value):
    digits = normalize_recovery_contact(value)
    return len(digits) == 10 and digits.isdigit()


def recovery_email_valid(value):
...
def approve_booking(booking_id):
    ...
    # Email is optional. It is disabled by default so approval does not
    # depend on Gmail/SMTP connectivity. When ENABLE_BOOKING_EMAIL=true,
    # the original "email must succeed before approval" behavior is used.
    if app.config.get("ENABLE_BOOKING_EMAIL", False):
    ...
    if app.config.get("ENABLE_BOOKING_EMAIL", False):
        ...
        flash(
            f"Booking #{booking.id} approved successfully. Email notifications are currently disabled.",
            "success"
        )
...
def reject_booking(booking_id):
    ...
    # Email is optional. When disabled, rejection proceeds immediately and
    # the reason is still stored on the booking. When enabled, the booking
    # remains pending unless the rejection email succeeds.
    if app.config.get("ENABLE_BOOKING_EMAIL", False):
        email_ok, email_message = _send_booking_email(
            booking,
            "rejected",
    ...
    if app.config.get("ENABLE_BOOKING_EMAIL", False):
        ...
        flash(
            f"Booking #{booking.id} rejected successfully. Email notifications are currently disabled.",
            "info"
        )
...
def google_login():
    ...
    try:
        ...
        if existing_email_user:
            ...
            # A recovered owner can sign in with the new Google account
            # after email migration. Attach the new Google ID to the
            # existing Goinn account instead of creating a second account.
            if (
                existing_email_user.role != "admin"
                and (
                    existing_email_user.google_id.startswith("recovered-")
                    or existing_email_user.role == "owner"
                    or Property.query.filter_by(
                        owner_id=existing_email_user.id
                    ).first()
                )
            ):
...
def user_bookings():
    ...
    # Keep the booking page focused on bookings only.
    # Categories:
    #   Upcoming  -> pending/availability_requested/approved stays
    #                 whose checkout is today or in the future.
    #   Completed -> past approved stays plus released/cancelled requests.
    #   Rejected  -> administrator/property-management rejected requests.
    bookings = (
        Booking.query
    ...
    for booking in bookings:
        ...
        if status == "rejected":
            ...
            # Released requests are historical/closed requests.
            completed_bookings.append(booking)

        elif status in {"pending", "availability_requested", "approved"}:
            ...
            # Keep any future/unknown booking state visible rather than
            # silently dropping it from the user's booking page.
            if booking.check_out and booking.check_out < today:
                completed_bookings.append(booking)
            else:
                upcoming_bookings.append(booking)
...
@app.route("/api/user/guest-details", methods=["POST"])
@login_required
def save_guest_details():
    """Save booking guest details to the signed-in user's profile."""
    ...
    current_user.contact = normalized_contact
...
def admin_dashboard():
    ...
    lost_account_requests = LostAccountRequest.query.filter(
        LostAccountRequest.status.in_(
            ["pending_admin", "code_generated"]
        )
    ).order_by(
        LostAccountRequest.id.desc()
    ).all()

    generated_lost_account_code = session.pop(
        "last_lost_account_code",
        None
    )
    ...
    return render_template(
        ...
        lost_account_requests=lost_account_requests,
        generated_lost_account_code=generated_lost_account_code
    )
...
# ==================================================
# LOST ACCOUNT / OWNER ACCOUNT MIGRATION
# ==================================================

@app.route("/lost-account", methods=["GET", "POST"])
def lost_account():

    if request.method == "GET":
        request_id = session.get("lost_account_request_id")
        recovery_user_id = session.get("lost_account_user_id")

        recovery_request = None
        recovery_user = None

        if request_id and recovery_user_id:
            recovery_request = db.session.get(
                LostAccountRequest,
                request_id
            )
            recovery_user = db.session.get(
                User,
                recovery_user_id
            )
            ...
            if (
                ...
                or recovery_request.user_id != recovery_user_id
                or not recovery_owner_user(recovery_user)
                or recovery_request.status == "completed"
            ):
                session.pop("lost_account_request_id", None)
                session.pop("lost_account_user_id", None)
                session.pop("lost_account_verified", None)
                recovery_request = None
                recovery_user = None
    ...
    previous_email = request.form.get(
        "previous_email", ""
    ).strip().lower()

    previous_contact = normalize_recovery_contact(
        request.form.get("previous_contact", "")
    )

    if not recovery_email_valid(previous_email):
        return render_template(
            "lost_account.html",
            error="Enter the previous email address used for the owner account."
        )

    if not valid_recovery_contact(previous_contact):
        return render_template(
            "lost_account.html",
            error="Enter the previous 10-digit contact number used for the owner account."
        )

    user = User.query.filter(
        db.func.lower(User.email) == previous_email
    ).first()

    if (
        not user
        or not recovery_owner_user(user)
        or normalize_recovery_contact(user.contact) != previous_contact
    ):
        return render_template(
            "lost_account.html",
            error="No active owner account matches the email and contact details provided."
        )

    active_request = LostAccountRequest.query.filter(
        LostAccountRequest.user_id == user.id,
        LostAccountRequest.status.in_(
            ["pending_admin", "code_generated"]
        )
    ).order_by(
    ...
    session["lost_account_user_id"] = user.id
    session["lost_account_verified"] = True

    if active_request:
        session["lost_account_request_id"] = active_request.id

        return render_template(
            "lost_account.html",
            recovery_request=active_request,
            recovery_user=user,
            message="Your account is already verified. Complete the recovery request below."
        )

    return render_template(
        "lost_account.html",
        recovery_user=user,
        message="Account is active. Enter the new email and/or contact details you want to use."
    )


@app.route("/lost-account/request", methods=["POST"])
def lost_account_request():

    if not session.get("lost_account_verified"):
        return redirect(url_for("lost_account"))

    user = db.session.get(
        User,
        session.get("lost_account_user_id")
    )

    if not user or not recovery_owner_user(user):
        session.pop("lost_account_user_id", None)
        session.pop("lost_account_verified", None)
        return redirect(url_for("lost_account"))

    new_email_raw = request.form.get(
        "new_email", ""
    ).strip().lower()

    new_contact_raw = request.form.get(
        "new_contact", ""
    ).strip()
    ...
    new_email = (
        new_email_raw
        if new_email_raw
        else (user.email or "").strip().lower()
    )

    new_contact = (
        normalize_recovery_contact(new_contact_raw)
        if new_contact_raw
        else normalize_recovery_contact(user.contact)
    )

    if new_email_raw and not recovery_email_valid(new_email):
        return render_template(
            "lost_account.html",
            recovery_user=user,
    ...
    if new_contact_raw and not valid_recovery_contact(new_contact_raw):
        return render_template(
            "lost_account.html",
            recovery_user=user,
            error="Enter a valid 10-digit new contact number."
        )

    if (
        new_email == (user.email or "").strip().lower()
        and new_contact == normalize_recovery_contact(user.contact)
    ):
        return render_template(
            "lost_account.html",
            recovery_user=user,
            error="Change at least the email or contact before sending the recovery request."
        )
    ...
    if existing:
        return render_template(
            "lost_account.html",
            recovery_user=user,
            error="That email is already associated with another Goinn account."
        )

    LostAccountRequest.query.filter(
        LostAccountRequest.user_id == user.id,
        LostAccountRequest.status.in_(
            ["pending_admin", "code_generated"]
        )
    ...
    recovery_request = LostAccountRequest(
        user_id=user.id,
        previous_email=user.email,
        previous_contact=user.contact or "",
        requested_email=new_email,
        requested_contact=new_contact,
    ...
    session["lost_account_request_id"] = recovery_request.id
    session["lost_account_user_id"] = user.id
    session["lost_account_verified"] = True
    ...
    return render_template(
        ...
        recovery_request=recovery_request,
        recovery_user=user,
        message="Recovery request sent to the Goinn admin. Keep this page open for the final 6-digit verification."
    )


@app.route(
    "/admin/lost-account/<int:request_id>/generate-code",
    methods=["POST"]
)
@login_required
def admin_generate_lost_account_code(request_id):
    ...
    if not recovery_request:
        flash("Lost account request not found.", "danger")
        return redirect(
            url_for(
                "admin_dashboard",
                tab="lost-account"
    ...
    if recovery_request.status not in [
        "pending_admin",
        "code_generated"
    ]:
        flash("This lost account request is no longer active.", "danger")
        return redirect(
            url_for(
                "admin_dashboard",
                tab="lost-account"
    ...
    if not user or not recovery_owner_user(user):
        recovery_request.status = "invalid"
        db.session.commit()
        flash(
            "The owner account is no longer eligible for recovery.",
            "danger"
        )
        return redirect(
            url_for(
    ...
    recovery_request.code_hash = recovery_code_hash(code)
    recovery_request.code_generated_at = datetime.utcnow()
    recovery_request.code_expires_at = (
        datetime.utcnow() + timedelta(minutes=30)
    )
    recovery_request.status = "code_generated"

    db.session.commit()

    session["last_lost_account_code"] = {
...
@app.route("/lost-account/verify-code", methods=["POST"])
def lost_account_verify_code():

    if not session.get("lost_account_verified"):
        return redirect(url_for("lost_account"))

    request_id = session.get("lost_account_request_id")
    user_id = session.get("lost_account_user_id")
    ...
    if (
        not recovery_request
        or recovery_request.user_id != user_id
        or not user
        or not recovery_owner_user(user)
    ):
        return render_template(
            "lost_account.html",
            error="The recovery session is no longer valid."
        )

    if recovery_request.status != "code_generated":
        return render_template(
            "lost_account.html",
            recovery_request=recovery_request,
            recovery_user=user,
            error="The administrator has not generated a verification code yet."
        )

    if (
        not recovery_request.code_expires_at
        or datetime.utcnow() > recovery_request.code_expires_at
    ):
        ...
        return render_template(
            ...
            recovery_request=recovery_request,
            recovery_user=user,
            error="This verification code has expired. Ask the administrator to generate a new code."
        )
    ...
    if len(code) != 6 or recovery_code_hash(code) != recovery_request.code_hash:
        return render_template(
            "lost_account.html",
            recovery_request=recovery_request,
            recovery_user=user,
    ...
    requested_email = (
        recovery_request.requested_email
        or user.email
    ).strip().lower()

    requested_contact = normalize_recovery_contact(
        recovery_request.requested_contact
        or user.contact
    )
    ...
    if email_owner:
        return render_template(
            ...
            recovery_request=recovery_request,
            recovery_user=user,
            error="The requested email is now in use by another account. Ask the administrator to create a new recovery request."
        )

    old_email = (user.email or "").strip().lower()

    user.email = requested_email
    user.contact = requested_contact

    # google_id is unique and required. Replace the old Google identity
    # with a temporary non-Google marker. The next Google login using the
    # migrated email will attach its real Google ID to this same account.
    if requested_email != old_email:
        user.google_id = (
            "recovered-" + secrets.token_urlsafe(24)
        )
    ...
    return render_template(
        "lost_account.html",
        completed=True,
        recovered_email=requested_email,
        recovered_contact=requested_contact
    )
